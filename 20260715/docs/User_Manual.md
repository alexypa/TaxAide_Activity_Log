...
Welcome to the AARP Tax-Aide Activity Logging and Tax Return Tracking System, a multi-site system designed as a collaborative tool for all AARP Tax-Aide volunteers engaged in providing tax preparation services. The primary goal in developing this system was to make the management of the tax preparation process at all sites as easy and less error-prone than activity logging systems used today, while providing leadership at the site and district level real-time status of the tax returns prepared by AARP Tax-Aide volunteers.

The system logs, tracks, documents and archives all activities of the tax preparation process from taxpayers appointments and check-in to the successful filing of their tax return, recording all steps taken during this process by all volunteers as well as all the exceptions that inevitably occur while providing tax preparation services to thousand of clients.

The system supplements rather than replaces the limited activity logging features provided by TaxSlayer, enabling AARP Tax-Aide volunteers to better manage the workflow during the tax filing season.

The system is based on the AARP-provided Google Workplace using Google Sheets and the Apps Script (GAS) scripting tool.

## 🗺️ Section 1: System Login

All AARP Tax-Aide volunteers engaged in providing tax preparation services access and collaborate by using the system. After logging into their Chromebook, the volunteer launches the system using the "Activity Logging" link provided on the Chrome browser's bookmark bar. No additional login is required.

## 🗺️ Section 2: Site Process Flow / Tax Return States

The overall process tracked by the system during the daily shift is depicted in  Figure 1.
...
An additional process, executed at the end of a day's shift, also described later, deals with the archiving of completed and uncompleted tax returns.

The process always begins by the taxpayer reporting to the Tax-Aide tax preparation site and met by a greeter (Support or Client Facilitator in AARP-speak) who checks them in by validating their identity and entering their information into the system. It always ends in one of four "terminal" states from which the system will not permit further transition. Those are:

* Accepted: A tax return that was electronically filed and accepted by the IRS
* Paper: A tax return that was prepared for the taxpayer to file by mail.
* Deactivated: A tax return that was deactivated in the TaxSlayer system
* No Return: The taxpayer and/or the Tax-Aide volunteer decided not to begin the process of preparing a tax return.

All other states are temporary and include:
...
For the following states the system enforces stating a reason for the tax return being assigned this state. The reason may be a combination of a preset, typical reason and/or free text entered by the operator. The reason will be recorded in the comments section of the tax return record (column J on the Activity Log tab). This requirement is important for maintaining communications amongst volunteers regarding the state of the tax return, as it may be handled by several volunteers with different roles over the course of several days.
...
### 2.1  Check-In Procedures at a Site Operating by Appointment

Prior to the daily session, the Session Management Coordinator populates the Appointments tab of the system with the day's appointments. This process will be described later in this manual. The Appointments tab is shown in Figure 2.
...
### 2.2  Check-In Procedures of a Walk-In Taxpayer
...
* Last name(s) (mandatory)
  * If the names are different from those recorded on the Appointment tab, the greeter should record the taxpayer(s) actual names on the Activity Log tab, starting with the primary taxpayer.
* Last 4 digits of the primary taxpayer's Social Security number
  * If required by the site leadership.
  * Inclusion or exclusion of this column in the system is controlled by the site's settings.
* Ticket # (optional)
  * Ticket numbers are used at walk-in sites to control the first-come/first-served queuing system
* Tax Year of the tax return
  * When one taxpayer request preparation of more than one year of tax returns, every year must be entered on a separate line
...
When a taxpayer is ready to be seen by a Counselor and a Counselor is available, the greeter will select a Counselor from the dropdown list (column E). The system will change the status of the tax return to "Assigned" (column I) and timestamp the "Latest Status Change" (column K). <span style="background-color: yellow; color: black">The row will be highlighted in yellow</span>. See Figure 5.
